...
const programs = [
  {
    ...
    subtitle: "Grades I - V",
    trackKey: "primary",
    description: "Building strong foundations through creativity, exploration, and joyful learning.",
    image: "https://thumbs.dreamstime.com/b/uk-school-classroom-children-working-33297853.jpg",
    icon: GraduationCap,
  ...
  },
  {
    ...
    trackKey: "middle",
    description: "Encouraging analytical thinking, innovation, collaboration, and leadership skills.",
    image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS29WMyp96EoqS5nksF2Oq87aHv_G1aFPd6pg&s",
    icon: BookOpen,
  ...
  },
  {
    ...
    trackKey: "high",
    description: "Preparing future leaders through advanced academics, research, and global exposure.",
    image: "https://admin.ssrvm.org/uploads/innovative_learning_spaces_fd5f1aab87.png",
    icon: Microscope,
    color: "from-red-700 to-red-900",
  },
];

// If you have a global state for the track, keep passing setSelectedTrack as a prop
function Programs({ setSelectedTrack }) {
  const navigate = useNavigate(); // 2. Initialize the navigation hook

  const handleNavigation = (trackId) => {
    if (setSelectedTrack) {
      setSelectedTrack(trackId); // Set data context first if function exists
    }
    navigate("/academics"); // 3. Programmatically push route to your Academics framework page
  };
          ...
          {programs.map((program, index) => {
            ...
            return (
              <motion.div
                ...
                transition={{
                  duration: 0.6,
                  delay: typeof window !== 'undefined' && window.innerWidth > 768 ? index * 0.15 : 0,
                }}
                viewport={{ once: true, margin: "-50px" }}
                whileHover={typeof window !== 'undefined' && window.innerWidth > 1024 ? { y: -12 } : {}}
              ...
              >
                ...
                <div className="relative overflow-hidden h-56 sm:h-64 lg:h-72 w-full flex-shrink-0">
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className={`absolute inset-0 bg-gradient-to-t ${program.color} opacity-35 sm:opacity-40`} />
                  <div className="absolute top-4 left-4 sm:top-6 sm:left-6 bg-white/20 backdrop-blur-md p-3 sm:p-4 rounded-xl sm:rounded-2xl">
                    <Icon size={28} className="text-white sm:w-[35px] sm:h-[35px]" />
                  </div>
                </div>
                ...
                <div className="p-6 sm:p-8 flex flex-col flex-grow justify-between">
                  ...
                  <div className="pt-6 sm:pt-8 mt-auto">
                    <button 
                      onClick={() => handleNavigation(program.trackKey)}
                      className="flex items-center gap-2 sm:gap-3 text-red-900 font-extrabold text-sm sm:text-base transition-all group-hover:gap-4 sm:group-hover:gap-5 duration-300"
                    >
                      Explore Program
        ...
        >
          <div className="relative z-10">
            ...
            <button 
              ...
              className="mt-6 sm:mt-8 w-full sm:w-auto bg-yellow-400 hover:bg-yellow-500 active:scale-[0.98] text-slate-950 font-black px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl duration-200 shadow-lg tracking-wide text-sm sm:text-base"
            >
              View Curriculum
            </button>
          </div>
